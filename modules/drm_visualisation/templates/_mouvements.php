...
<?php if (count($mouvementsByProduit) > 0): ?>
    ...
    <table id="table_mouvements" class="table table-striped table-condensed">
        ...
        <tbody>
            <?php $i = 1; ?>
            <?php
            $correspondances = DRMClient::$correspondance_types;
            foreach (array_keys($mouvementsByProduit->getRawValue()) as $type):
            	foreach ($mouvementsByProduit[$type] as $produit_hash => $mouvements):
                $produit = $drm->get($produit_hash);
                $produit_libelle = $produit->getLibelle();
                $libelleDoc = DRMClient::getInstance()->getLibelleFromId($drm->_id);
                ?>
                <?php foreach($produit->getProduitsDetails(true, $correspondances[$type]) as $detail): ?>
